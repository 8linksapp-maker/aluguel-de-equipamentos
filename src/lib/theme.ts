...
import equipamentos from '../data/themes/equipamentos.json';
...
export const PRESETS: Record<string, ThemePreset> = { dental, harmonizacao, vidro, equipamentos };
...
export function resolveTheme(): { vars: Record<string, string>; preset: ThemePreset } {
  const preset = PRESETS[themeConfig.preset] ?? PRESETS.equipamentos;
